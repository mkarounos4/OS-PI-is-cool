...
// Stubs for symbols that os-helpers expects the main program to provide.
// Standalone PennFAT does not run the PennOS scheduler or manage jobs, so
// these are no-ops sufficient to satisfy the linker.
void exit_shell() {
}
...
void execute_commands(struct parsed_command *parsed_cmd, char *cmd) {
    ...
    if (parsed_cmd->num_commands == 1) {
        ...
        if (check_fat_commands(parsed_cmd->commands[0])) {
            return;
        }
        pid_t pid = fork();
