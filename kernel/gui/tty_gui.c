#include "gui.h"
#include "tty_gui.h"
#include "ascii_font.h"
#include "devices/tty.h"
#include "memory/kmalloc.h"
#include "string.h"

int MAX_ROWS;
int MAX_COLS;
#define MAIN_COLOR (uint32_t) gui_framebuffer_encode_color(0xFF, 0, 0xFF)
#define BG_COLOR (uint32_t) gui_framebuffer_encode_color(0x18, 0, 0x32)
#define CHAR_HEIGHT 16
#define CHAR_WIDTH 8
#define WIDTH_BUFFER 0
#define HEIGHT_BUFFER 0
#define TAB_BAR_HEIGHT 24
#define TAB_WIDTH 96
#define TAB_PADDING_X 10
#define TAB_LABEL_Y 4
#define TTY_GUI_TERMINALS MAX_TTY_DEVICES
#define TTY_GUI_MAX_ROWS 80
#define TTY_GUI_MAX_COLS 160
#define TAB_BAR_COLOR (uint32_t) gui_framebuffer_encode_color(0x2b, 0x2b, 0x3c)
#define TAB_ACTIVE_COLOR (uint32_t) gui_framebuffer_encode_color(0x18, 0, 0x32)
#define TAB_INACTIVE_COLOR (uint32_t) gui_framebuffer_encode_color(0x42, 0x42, 0x55)
#define TAB_BORDER_COLOR (uint32_t) gui_framebuffer_encode_color(0x7d, 0x7d, 0x96)
#define TAB_TEXT_COLOR (uint32_t) gui_framebuffer_encode_color(0xff, 0xff, 0xff)

typedef struct tty_gui_terminal_st {
    int cursor_row;
    int cursor_col;
    int visible;
    char *cells;
} tty_gui_terminal_t;

static tty_gui_terminal_t terminals[TTY_GUI_TERMINALS];
static int active_terminal = -1;

void toggle_cursor(void);

static uint32_t row_to_px(int row) {
    return TAB_BAR_HEIGHT + HEIGHT_BUFFER + row * (CHAR_HEIGHT + HEIGHT_BUFFER);
}

static uint32_t col_to_px(int col) {
    return WIDTH_BUFFER + col * (CHAR_WIDTH + WIDTH_BUFFER);
}

static tty_gui_terminal_t *terminal_for_index(int terminal) {
    if (terminal < 0 || terminal >= TTY_GUI_TERMINALS) {
        terminal = active_terminal;
    }

    return &terminals[terminal];
}

static tty_gui_terminal_t *active_state(void) {
    if (active_terminal < 0 || active_terminal >= TTY_GUI_TERMINALS ||
        !terminals[active_terminal].visible) {
        return NULL;
    }
    return &terminals[active_terminal];
}

static size_t cell_index(int row, int col) {
    return (size_t)row * TTY_GUI_MAX_COLS + (size_t)col;
}

static int active_terminal_is_valid(void) {
    return active_terminal >= 0 && active_terminal < TTY_GUI_TERMINALS &&
           terminals[active_terminal].visible;
}

static int tty_dimensions_valid(void) {
    return MAX_ROWS > 0 && MAX_ROWS <= TTY_GUI_MAX_ROWS &&
           MAX_COLS > 0 && MAX_COLS <= TTY_GUI_MAX_COLS;
}

static void normalize_cursor(tty_gui_terminal_t *tty) {
    if (!tty_dimensions_valid()) {
        return;
    }
    if (tty->cursor_row < 0 || tty->cursor_row >= MAX_ROWS) {
        tty->cursor_row = MAX_ROWS - 1;
    }
    if (tty->cursor_col < 0 || tty->cursor_col >= MAX_COLS) {
        tty->cursor_col = MAX_COLS - 1;
    }
}

static void draw_char_pixels(uint32_t x, uint32_t y, char c,
                             uint32_t fg, uint32_t bg) {
    uint8_t *char_format = get_ascii_char_font(c);
    for (int i = 0; i < CHAR_WIDTH; i++) {
        for (int j = 0; j < CHAR_HEIGHT; j++) {
            gui_framebuffer_put_pixel_encoded(x + (uint32_t)i,
                                              y + (uint32_t)j,
                                              (char_format[j] & (1 << (7 - i))) ?
                                                  fg : bg);
        }
    }
}

static void draw_char_at(int row, int col, char c) {
    draw_char_pixels(col_to_px(col), row_to_px(row), c, MAIN_COLOR, BG_COLOR);
}

static void fill_rect(uint32_t x, uint32_t y, uint32_t w, uint32_t h,
                      uint32_t color) {
    for (uint32_t row = y; row < y + h; row++) {
        for (uint32_t col = x; col < x + w; col++) {
            gui_framebuffer_put_pixel_encoded(col, row, color);
        }
    }
}

static void draw_tab_label(int tab, uint32_t x, uint32_t y, uint32_t bg) {
    char label[8];
    label[0] = 't';
    label[1] = 't';
    label[2] = 'y';
    int len = 3;
    if (tab >= 10) {
        label[len++] = (char)('0' + (tab / 10));
    }
    label[len++] = (char)('0' + (tab % 10));
    label[len] = '\0';

    for (int i = 0; label[i] != '\0'; i++) {
        draw_char_pixels(x + (uint32_t)(i * CHAR_WIDTH), y, label[i],
                         TAB_TEXT_COLOR, bg);
    }
}

static void draw_tab_bar(void) {
    gui_framebuffer_t fb;
    if (!gui_framebuffer_get(&fb)) {
        return;
    }

    fill_rect(0, 0, fb.width, TAB_BAR_HEIGHT, TAB_BAR_COLOR);
    int visible_tab = 0;
    for (int tab = 0; tab < TTY_GUI_TERMINALS; tab++) {
        if (!terminals[tab].visible) {
            continue;
        }

        uint32_t x = (uint32_t)(visible_tab * TAB_WIDTH);
        if (x >= fb.width) {
            break;
        }
        uint32_t tab_width = TAB_WIDTH;
        if (x + tab_width > fb.width) {
            tab_width = fb.width - x;
        }

        uint32_t color = tab == active_terminal ?
            TAB_ACTIVE_COLOR : TAB_INACTIVE_COLOR;
        fill_rect(x, 1, tab_width, TAB_BAR_HEIGHT - 1, color);
        fill_rect(x, TAB_BAR_HEIGHT - 1, tab_width, 1, TAB_BORDER_COLOR);
        fill_rect(x, 1, 1, TAB_BAR_HEIGHT - 1, TAB_BORDER_COLOR);
        fill_rect(x + tab_width - 1, 1, 1, TAB_BAR_HEIGHT - 1, TAB_BORDER_COLOR);
        draw_tab_label(tab, x + TAB_PADDING_X, TAB_LABEL_Y, color);
        visible_tab++;
    }
}

static void redraw_active_terminal(void) {
    if (!tty_dimensions_valid() || !active_terminal_is_valid()) {
        gui_framebuffer_clear(0x18, 0, 0x32);
        draw_tab_bar();
        return;
    }

    tty_gui_terminal_t *tty = active_state();

    gui_framebuffer_clear(0x18, 0, 0x32);
    draw_tab_bar();
    for (int row = 0; row < MAX_ROWS; row++) {
        for (int col = 0; col < MAX_COLS; col++) {
            char c = tty->cells != NULL ? tty->cells[cell_index(row, col)] : ' ';
            if (c != ' ' && c != '\0') {
                draw_char_at(row, col, c);
            }
        }
    }
}

static void scroll_down_row(tty_gui_terminal_t *tty) {
    if (tty == NULL || !tty_dimensions_valid() || tty->cells == NULL) {
        return;
    }

    normalize_cursor(tty);

    for (int row = 1; row < MAX_ROWS; row++) {
        memcpy(&tty->cells[cell_index(row - 1, 0)],
               &tty->cells[cell_index(row, 0)],
               (size_t)MAX_COLS);
    }
    memset(&tty->cells[cell_index(MAX_ROWS - 1, 0)], ' ', (size_t)MAX_COLS);

    if (tty == active_state()) {
        redraw_active_terminal();
    }
}

void init_tty_gui(void) {
    gui_framebuffer_t fb;
    int success = gui_framebuffer_get(&fb);
    if (!success) {
        return;
    }

    MAX_COLS = fb.width / (CHAR_WIDTH + WIDTH_BUFFER);
    MAX_ROWS = (fb.height - TAB_BAR_HEIGHT) / (CHAR_HEIGHT + HEIGHT_BUFFER);
    if (MAX_COLS > TTY_GUI_MAX_COLS) {
        MAX_COLS = TTY_GUI_MAX_COLS;
    }
    if (MAX_ROWS > TTY_GUI_MAX_ROWS) {
        MAX_ROWS = TTY_GUI_MAX_ROWS;
    }

    active_terminal = -1;
    for (int i = 0; i < TTY_GUI_TERMINALS; i++) {
        terminals[i].cursor_row = 0;
        terminals[i].cursor_col = 0;
        terminals[i].visible = 0;
        terminals[i].cells = NULL;
    }

    gui_framebuffer_clear(0x18, 0, 0x32);
    draw_tab_bar();
}

int tty_gui_create_terminal(int terminal) {
    if (terminal < 0 || terminal >= TTY_GUI_TERMINALS) {
        return -1;
    }

    if (terminals[terminal].cells == NULL) {
        terminals[terminal].cells = kmalloc(TTY_GUI_MAX_ROWS * TTY_GUI_MAX_COLS);
        if (terminals[terminal].cells == NULL) {
            return -1;
        }
    }

    terminals[terminal].cursor_row = 0;
    terminals[terminal].cursor_col = 0;
    terminals[terminal].visible = 1;
    memset(terminals[terminal].cells, ' ', TTY_GUI_MAX_ROWS * TTY_GUI_MAX_COLS);
    if (!active_terminal_is_valid()) {
        active_terminal = terminal;
    }
    redraw_active_terminal();
    toggle_cursor();
    return 0;
}

void tty_gui_destroy_terminal(int terminal) {
    if (terminal < 0 || terminal >= TTY_GUI_TERMINALS) {
        return;
    }

    if (terminals[terminal].visible) {
        tty_gui_set_terminal_visible(terminal, 0);
    }
    if (terminals[terminal].cells != NULL) {
        kfree(terminals[terminal].cells);
        terminals[terminal].cells = NULL;
    }
    terminals[terminal].cursor_row = 0;
    terminals[terminal].cursor_col = 0;
}

void toggle_cursor(void) {
    if (!tty_dimensions_valid() || !active_terminal_is_valid()) {
        return;
    }

    tty_gui_terminal_t *tty = active_state();
    normalize_cursor(tty);

    for (uint32_t i = row_to_px(tty->cursor_row); i < row_to_px(tty->cursor_row) + 16; i++) {
        for (uint32_t j = col_to_px(tty->cursor_col); j < col_to_px(tty->cursor_col) + 8; j++) {
            uint32_t curr_color;
            int success = gui_framebuffer_get_pixel(j, i, &curr_color);
            if (!success) {
                return;
            }

            success = gui_framebuffer_put_pixel_encoded(j, i, curr_color == MAIN_COLOR ? BG_COLOR : MAIN_COLOR);
            if (!success) {
                return;
            }
        }
    }
}

static void cursor_advance_row(tty_gui_terminal_t *tty) {
    if (tty == NULL || !tty_dimensions_valid()) {
        return;
    }
    normalize_cursor(tty);
    if (tty == active_state()) {
        toggle_cursor();
    }

    if (tty->cursor_row == MAX_ROWS - 1) {
        scroll_down_row(tty);
        tty->cursor_col = 0;
    } else {
        tty->cursor_col = 0;
        tty->cursor_row++;
    }

    if (tty == active_state()) {
        toggle_cursor();
    }
}

static void advance_cursor(tty_gui_terminal_t *tty) {
    if (tty == NULL || !tty_dimensions_valid()) {
        return;
    }
    normalize_cursor(tty);
    tty->cursor_col++;
    if (tty->cursor_col == MAX_COLS) {
        tty->cursor_col = 0;
        tty->cursor_row++;
        
        if (tty->cursor_row == MAX_ROWS) {
            scroll_down_row(tty);
            tty->cursor_row--;
        }
    }
}

void tty_gui_cursor_left(void) {
    if (!tty_dimensions_valid() || !active_terminal_is_valid()) {
        return;
    }
    tty_gui_terminal_t *tty = active_state();
    normalize_cursor(tty);

    if (tty->cursor_row == 0 && tty->cursor_col == 0) {
        return;
    }

    toggle_cursor();
    if (tty->cursor_col == 0) {
        tty->cursor_row--;
        tty->cursor_col = MAX_COLS - 1;
    } else {
        tty->cursor_col--;
    }
    toggle_cursor();
}

void tty_gui_cursor_right(void) {
    if (!tty_dimensions_valid() || !active_terminal_is_valid()) {
        return;
    }
    tty_gui_terminal_t *tty = active_state();
    normalize_cursor(tty);

    if (tty->cursor_row == MAX_ROWS - 1 && tty->cursor_col == MAX_COLS - 1) {
        return;
    }

    toggle_cursor();
    tty->cursor_col++;
    if (tty->cursor_col == MAX_COLS) {
        tty->cursor_col = 0;
        tty->cursor_row++;
    }
    toggle_cursor();
}

void backtrack_cursor(void) {
    tty_gui_cursor_left();
}

static void clear_screen(void) {
    tty_gui_terminal_t *tty = active_state();
    if (tty == NULL) {
        return;
    }

    tty->cursor_row = 0;
    tty->cursor_col = 0;
    if (tty->cells != NULL) {
        memset(tty->cells, ' ', TTY_GUI_MAX_ROWS * TTY_GUI_MAX_COLS);
    }
    redraw_active_terminal();
    toggle_cursor();
}

static void write_tab(void) {
    tty_gui_terminal_t *tty = active_state();
    if (tty == NULL) {
        return;
    }
    int target_col = ((tty->cursor_col / 8) + 1) * 8;
    do {
        tty_gui_write_char(' ');
    } while (tty->cursor_col != 0 && tty->cursor_col < target_col);
}

void tty_gui_write_char(const char c) {
    tty_gui_write_char_for_tty(active_terminal, c);
}

void tty_gui_write_char_for_tty(int terminal, const char c) {
    if (terminal < 0 || terminal >= TTY_GUI_TERMINALS ||
        !terminals[terminal].visible || !tty_dimensions_valid() ||
        terminals[terminal].cells == NULL) {
        return;
    }

    tty_gui_terminal_t *tty = terminal_for_index(terminal);
    normalize_cursor(tty);
    int is_active = tty == active_state();

    if (c == '\n') {
        cursor_advance_row(tty);
    } else if (c == '\b') {
        if (is_active) {
            backtrack_cursor();
        } else if (tty->cursor_row != 0 || tty->cursor_col != 0) {
            if (tty->cursor_col == 0) {
                tty->cursor_row--;
                tty->cursor_col = MAX_COLS - 1;
            } else {
                tty->cursor_col--;
            }
        }
    } else if (c == '\f') {
        if (is_active) {
            clear_screen();
        } else {
            tty->cursor_row = 0;
            tty->cursor_col = 0;
            if (tty->cells != NULL) {
                memset(tty->cells, ' ', TTY_GUI_MAX_ROWS * TTY_GUI_MAX_COLS);
            }
        }
    } else if (c == '\t') {
        if (is_active) {
            write_tab();
        }
    } else {
        if (is_active) {
            toggle_cursor();
        }

        if (tty->cells != NULL) {
            tty->cells[cell_index(tty->cursor_row, tty->cursor_col)] = c;
        }
        if (is_active) {
            draw_char_at(tty->cursor_row, tty->cursor_col, c);
        }
        advance_cursor(tty);

        if (is_active) {
            toggle_cursor();
        }
    }
}

void tty_gui_activate_terminal(int terminal) {
    if (terminal < 0 || terminal >= TTY_GUI_TERMINALS ||
        !terminals[terminal].visible ||
        terminal == active_terminal) {
        return;
    }

    active_terminal = terminal;
    redraw_active_terminal();
    toggle_cursor();
}

int tty_gui_get_active_terminal(void) {
    return active_terminal;
}

void tty_gui_set_terminal_visible(int terminal, int visible) {
    if (terminal < 0 || terminal >= TTY_GUI_TERMINALS) {
        return;
    }

    if (visible && terminals[terminal].cells == NULL) {
        tty_gui_create_terminal(terminal);
        return;
    }

    terminals[terminal].visible = visible ? 1 : 0;
    if (visible) {
        if (!active_terminal_is_valid()) {
            active_terminal = terminal;
        }
        redraw_active_terminal();
        toggle_cursor();
        return;
    }

    if (active_terminal == terminal) {
        int next = tty_gui_next_visible_terminal(terminal, 1);
        if (next < 0) {
            next = tty_gui_next_visible_terminal(terminal, -1);
        }
        if (next >= 0) {
            active_terminal = next;
        }
    }
    redraw_active_terminal();
    toggle_cursor();
}

int tty_gui_is_terminal_visible(int terminal) {
    return terminal >= 0 && terminal < TTY_GUI_TERMINALS &&
           terminals[terminal].visible;
}

int tty_gui_next_visible_terminal(int from, int delta) {
    if (delta == 0) {
        return tty_gui_is_terminal_visible(from) ? from : -1;
    }

    int step = delta < 0 ? -1 : 1;
    for (int i = 1; i <= TTY_GUI_TERMINALS; i++) {
        int next = (from + (i * step)) % TTY_GUI_TERMINALS;
        if (next < 0) {
            next += TTY_GUI_TERMINALS;
        }
        if (terminals[next].visible) {
            return next;
        }
    }
    return -1;
}

int tty_gui_get_rows(void) {
    return MAX_ROWS;
}

int tty_gui_get_cols(void) {
    return MAX_COLS;
}

int tty_gui_get_cursor_row(void) {
    tty_gui_terminal_t *tty = active_state();
    return tty == NULL ? 0 : tty->cursor_row;
}

int tty_gui_get_cursor_col(void) {
    tty_gui_terminal_t *tty = active_state();
    return tty == NULL ? 0 : tty->cursor_col;
}

size_t tty_gui_screen_size(void) {
    return (size_t)MAX_ROWS * (size_t)MAX_COLS;
}

static int tty_gui_copy_cells(int terminal, char *cells, size_t count) {
    if (cells == NULL || terminal < 0 || terminal >= TTY_GUI_TERMINALS ||
        !terminals[terminal].visible || terminals[terminal].cells == NULL ||
        count < tty_gui_screen_size()) {
        return -1;
    }

    for (int row = 0; row < MAX_ROWS; row++) {
        memcpy(cells + ((size_t)row * (size_t)MAX_COLS),
               terminals[terminal].cells + cell_index(row, 0),
               (size_t)MAX_COLS);
    }
    return 0;
}

int tty_gui_copy_screen(int terminal, char *cells, size_t count,
                        int *cursor_row, int *cursor_col) {
    if (cursor_row == NULL || cursor_col == NULL ||
        tty_gui_copy_cells(terminal, cells, count) != 0) {
        return -1;
    }

    *cursor_row = terminals[terminal].cursor_row;
    *cursor_col = terminals[terminal].cursor_col;
    return 0;
}

int tty_gui_present_screen(int terminal, const char *cells, size_t count,
                           int cursor_row, int cursor_col) {
    if (cells == NULL || terminal < 0 || terminal >= TTY_GUI_TERMINALS ||
        !terminals[terminal].visible || terminals[terminal].cells == NULL ||
        count < tty_gui_screen_size() || cursor_row < 0 ||
        cursor_row >= MAX_ROWS || cursor_col < 0 || cursor_col >= MAX_COLS) {
        return -1;
    }

    for (int row = 0; row < MAX_ROWS; row++) {
        memcpy(terminals[terminal].cells + cell_index(row, 0),
               cells + ((size_t)row * (size_t)MAX_COLS),
               (size_t)MAX_COLS);
    }
    terminals[terminal].cursor_row = cursor_row;
    terminals[terminal].cursor_col = cursor_col;
    if (terminal == active_terminal) {
        redraw_active_terminal();
        toggle_cursor();
    }
    return 0;
}

int tty_gui_restore_screen(int terminal, const char *cells, size_t count,
                           int cursor_row, int cursor_col) {
    return tty_gui_present_screen(terminal, cells, count,
                                  cursor_row, cursor_col);
}
